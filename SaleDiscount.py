import logging
import OfferDiscount

logging.basicConfig(filename="SaleDiscount.log",level=logging.DEBUG,format='%(asctime)s :%(filename)s :%(levelname)s :%(message)s')
...
if ticket <= 4000:
    ...
    logging.debug(f"Your ticket number is {ticket} and you will get discount of INR-{dis} {dis_per}-off on total sale.")
elif ticket <= 8000:
    dis=int(sales*0.25)
    dis_per="25%"
    print(f"Your ticket number is {ticket} and you will get discount of INR-{dis} {dis_per}-off on total sale.")
